<template>
  <div class="page-main">
    <div class="filte-box">
      <el-form :model="filterParams" :inline="true" ref="filterRef">
        <el-form-item label="名称" prop="name"
          ><el-input placeholder="付款方式名称" v-model="filterParams.name" style="width: 220px"
        /></el-form-item>
    ...
    <div class="flex justify-between my-4">
      ...
      <el-button size="small" type="danger" @click="handleAdd"><i class="iconfont iconadd"></i>创建付款方式</el-button>
    </div>
    <div>
      <el-table
        :data="list"
...
import { getPayMethodList, addPayMethod, upPayMethod, delPayMethod } from '@/api'
...
const { data: list, count: total, loading } = useTable(getPayMethodList, filterQuery, page, limit, refresh)

const handleAdd = () => {
  dialogTitle.value = '创建付款方式'
  visible.value = true
}

const handleEditItem = (row) => {
  editFormData.value = row
  dialogTitle.value = `编辑付款方式：${row.name}`
  visible.value = true
}
...
const handleSubmit = async () => {
  ...
  const API = id ? upPayMethod : addPayMethod
