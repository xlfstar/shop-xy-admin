<template>
  <div class="container-xy relative">
    <div class="absolute spec-set right-0 top-0">
      <el-button type="danger" @click="visible = true">规格设置</el-button>
    </div>
...
import { getCategories, uploadImg, addGoods, addSkus } from '@/api'
  ...
  await el.validate(async (valid, fields) => {
    if (valid) {
      try {
        ...
        loading.value = true
        const { data, code, message } = await addGoods({ body })
